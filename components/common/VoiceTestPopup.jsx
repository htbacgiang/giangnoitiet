...
const VoiceTestPopup = ({ isOpen, onClose }) => {
  ...
  // MC Script content
  const mcScript = `KỊCH BẢN TEST GIỌNG MC

1. GIỚI THIỆU BẢN THÂN (30 giây)
"Xin chào, tôi là [Tên của bạn], [Tuổi] tuổi, hiện đang [Nghề nghiệp/Công việc]. Tôi rất quan tâm đến việc phát triển kỹ năng MC và tin rằng BT Academy sẽ giúp tôi cải thiện khả năng dẫn chương trình của mình."
...
4. KẾT THÚC (30 giây)
"Cảm ơn các bạn đã lắng nghe. Tôi hy vọng phần thể hiện này sẽ giúp BT Academy đánh giá được tiềm năng của tôi. Tôi rất mong nhận được phản hồi và lộ trình phát triển phù hợp. Xin chào và hẹn gặp lại!"
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-6xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row h-[calc(95vh-80px)] sm:h-[70vh]">
          ...
          <div className="w-full sm:w-1/2 p-3 sm:p-6 flex flex-col">
              ...
              <div className="bg-gray-50 rounded-lg p-2 sm:p-3">
                ...
                {!selectedFile && !audioBlob && (
                  <button
                    ...
                    className="w-full bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg flex items-center justify-center space-x-2 transition-colors text-sm"
                  >
                    <Upload size={16} />
                    <span>Chọn file</span>
                  </button>
            ...
            {/* Error Message */}
            {errorMessage && (
              <div className="mb-3 bg-red-100 text-red-800 px-3 py-2 rounded-lg flex items-center space-x-2 text-sm">
            ...
            <div className="mt-auto sticky bottom-0 bg-white pt-1 sm:pt-0">
              {isSubmitted ? (
                <div className="bg-green-100 text-green-800 px-3 py-2 rounded-lg flex items-center justify-center space-x-2 text-xs sm:text-sm">
